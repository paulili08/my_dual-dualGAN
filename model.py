...
class DualNet(object):
    def __init__(self, sess, image_size=256, batch_size=1, gcn=64, dcn=64, \
                 A_channels=3, B_channels=3, dataset_name='eeg2audio', participant='sub-01', \
        ...
        self.dir_name = "%s-%s-img_sz_%s-fltr_dim_%d-%s-lambda_AB_%s_%s" % (
            self.dataset_name,
            self.participant,
            self.image_size,
            self.gcn,
    ...
    def sample_shotcut(self, sample_dir, participant, epoch_idx, batch_idx):
        ...
        save_images(A2B_imgs, [self.batch_size, 1],
                    './{}/{}/{}/{:06d}_{:04d}_A2B.jpg'.format(sample_dir, participant, self.dir_name, epoch_idx, batch_idx))
        save_images(A2B2A_imgs, [self.batch_size, 1],
                    './{}/{}/{}/{:06d}_{:04d}_A2B2A.jpg'.format(sample_dir, participant, self.dir_name, epoch_idx, batch_idx))

        save_images(B2A_imgs, [self.batch_size, 1],
                    './{}/{}/{}/{:06d}_{:04d}_B2A.jpg'.format(sample_dir, participant, self.dir_name, epoch_idx, batch_idx))
        save_images(B2A2B_imgs, [self.batch_size, 1],
                    './{}/{}/{}/{:06d}_{:04d}_B2A2B.jpg'.format(sample_dir, participant, self.dir_name, epoch_idx, batch_idx))
    ...
    def train(self, args):
        ...
        for epoch_idx in range(args.epoch):
            data_A = glob('./datasets/{}/train/{}/A/*.*[g|G]'.format(self.dataset_name, self.participant))

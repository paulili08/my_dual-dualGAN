...
def extractMelSpecs(audio, sr, feat_col, windowLength=0.05, frameshift=0.01):
    """
    ...
    for w in range(numWindows):
        ...
        spectrogram[w, :] = spec
    mfb = mel.MelFilterBank(spectrogram.shape[1], feat_col, sr)
...
if __name__ == "__main__":
    ...
    for p_id, participant in enumerate(participants['participant_id']):
        ...
        channels = pd.read_csv(
            os.path.join(path_bids, participant, 'ieeg', f'{participant}_task-wordProduction_channels.tsv'),
            delimiter='\t')
        ...
        # Fit PCA to feature
        pca.fit(feat)
        # Transform data into component space (first 128)
        feat = np.dot(feat, pca.components_[:numComps, :].T)
        ...
        # Extract spectrogram
        melSpec = extractMelSpecs(scaled, audio_sr, numComps, windowLength=winL, frameshift=frameshift)

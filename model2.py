...
class DualNet(object):
    ...
    def load_random_samples(self):
        ...
        sample_files = np.random.choice(
            glob('./datasets/{}/val/{}/C/*.*[g|G]'.format(self.dataset_name, self.participant)), self.batch_size)
        ...
        sample_C_imgs = np.reshape(np.array(sample_C_imgs).astype(np.float32),
                                   (self.batch_size, self.image_size, self.image_size, -1))
        return sample_B_imgs, sample_C_imgs

    def sample_shotcut(self, sample_dir, participant, epoch_idx, batch_idx):
        sample_B_imgs, sample_C_imgs = self.load_random_samples()

        Bg, B2C2B_imgs, B2C_imgs = self.sess.run([self.B_loss, self.B2C2B, self.B2C],
                                                 feed_dict={self.real_B: sample_B_imgs, self.real_C: sample_C_imgs})
        Cg, C2B2C_imgs, C2B_imgs = self.sess.run([self.C_loss, self.C2B2C, self.C2B],
                                                 feed_dict={self.real_B: sample_B_imgs, self.real_C: sample_C_imgs})

        save_images(B2C_imgs, [self.batch_size, 1],
                    './{}/{}/{}/{:06d}_{:04d}_B2C.jpg'.format(sample_dir, participant, self.dir_name, epoch_idx, batch_idx))
        save_images(B2C2B_imgs, [self.batch_size, 1],
                    './{}/{}/{}/{:06d}_{:04d}_B2C2B.jpg'.format(sample_dir, participant, self.dir_name, epoch_idx, batch_idx))

        save_images(C2B_imgs, [self.batch_size, 1],
                    './{}/{}/{}/{:06d}_{:04d}_C2B.jpg'.format(sample_dir, participant, self.dir_name, epoch_idx, batch_idx))
        save_images(C2B2C_imgs, [self.batch_size, 1],
                    './{}/{}/{}/{:06d}_{:04d}_C2B2C.jpg'.format(sample_dir, participant, self.dir_name, epoch_idx, batch_idx))
    ...
    def train(self, args):
        ...
        self.writer = tf.summary.FileWriter("./logs/" + self.participant + self.dir_name, self.sess.graph)
        ...
        for epoch_idx in range(args.epoch):
            data_B = glob('./datasets/{}/train/{}/B/*.*[g|G]'.format(self.dataset_name, self.participant))
            data_C = glob('./datasets/{}/train/{}/C/*.*[g|G]'.format(self.dataset_name, self.participant))
            ...
            for batch_idx in range(0, epoch_size):
                ...
                if np.mod(step, 100) == 1:
                    self.sample_shotcut(args.sample_dir, args.participant, epoch_idx, batch_idx)
    ...
    def run_optim(self, batch_B_imgs, batch_C_imgs, counter, start_time, batch_idx):
        ...
        _, Bdfake, Bdreal, Cdfake, Cdreal, Bd, Cd = self.sess.run(
            [self.d_optim, self.Bd_loss_fake, self.Bd_loss_real, self.Cd_loss_fake, self.Cd_loss_real, self.Bd_loss,
             self.Cd_loss],
    ...
    def fcn(self, imgs, prefix=None, reuse=False):
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            ...
            if prefix == 'C_g_':
                ...
                self.d8, self.d8_w, self.d8_b = deconv2d(tf.nn.relu(d7), [self.batch_size, s, s, self.C_channels],
                                                         k_h=5, k_w=5, name=prefix + 'd8', with_w=True)
    ...
    def save(self, checkpoint_dir, step):
        ...
        sub_checkpoint_dir = os.path.join(checkpoint_dir, participant)

        if not os.path.exists(sub_checkpoint_dir):
            os.makedirs(sub_checkpoint_dir)

        checkpoint_dir = os.path.join(sub_checkpoint_dir, model_dir)
    ...
    def test(self, args):
        ...
        if self.load(self.checkpoint_dir):
            print(" [*] Load SUCCESS")
            test_dir = './{}/{}/{}'.format(args.test_dir, args.participant, self.dir_name)
    ...
    def test_domain(self, args, test_log, type='B'):
        test_files = glob('./datasets/{}/val/{}/{}/*.*[g|G]'.format(self.dataset_name, self.participant, type))
        # load testing input
        ...
        test_path = './{}/{}/{}/'.format(args.test_dir, args.participant, self.dir_name)
        # test input samples
        if type == 'B':

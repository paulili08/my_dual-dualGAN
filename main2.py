...
parser.add_argument('--image_size', dest='image_size', type=int, default=256,
                    help='size of input images (applicable to both B images and C images)')
...
parser.add_argument('--GAN_type', dest='GAN_type', type=str, default='wgan-gp', choices=['wgan-gp', 'wgan', 'classic'],
                    help='wgan gp | wgan | classic')
parser.add_argument('--clip_value', dest='clip_value', type=float, default=0.01, help='clip values')
parser.add_argument('--n_critic', dest='n_critic', type=int, default=1,
                    help='train discriminators # times per generator training')
parser.add_argument('--disc_type', dest='disc_type', type=str, default='globalgan', choices=['globalgan', 'patchgan'],
...
def main(_):
    ...
    sub_checkpoint_dir = args.checkpoint_dir + '/' + args.participant + '/'
    sub_sample_dir = args.sample_dir + '/' + args.participant + '/'
    sub_test_dir = args.test_dir + '/' + args.participant + '/'
    sub_log_dir = './logs/' + args.participant + '/'
    if not os.path.exists(sub_checkpoint_dir):
        os.makedirs(sub_checkpoint_dir)
    if not os.path.exists(sub_sample_dir):
        os.makedirs(sub_sample_dir)
    ...
    with tf.Session() as sess:
        model = DualNet(sess, image_size=args.image_size, batch_size=args.batch_size, \
                        dataset_name=args.dataset_name, participant=args.participant, B_channels=args.B_channels, \
